...
def carregar_dados():
    try:
        ...
        # --- ALTERAÇÃO: FILTRAR PARA OS ÚLTIMOS 6 MESES ---
        # Determina a data mais recente no dataset
        data_mais_recente = df['data_hora'].max()
        # Calcula a data de início (6 meses antes da data mais recente)
        data_inicio_6_meses = data_mais_recente - pd.DateOffset(months=6)
        # Filtra o dataframe para conter apenas os últimos 6 meses
        ...
        # --- ALTERAÇÃO: EXTRAIR NOME DO MÊS PARA O FILTRO ---
...
if df_acidentes is not None:
    ...
    # --- NOVO FILTRO: FILTRO POR MÊS ---
    # Ordena os meses corretamente para o filtro
    meses_disponiveis = sorted(df_acidentes['mes'].unique(), key=lambda x: list(pd.to_datetime(df_acidentes['data_hora']).dt.month.unique()))
    ...
    # --- ALTERAÇÃO: APLICA O FILTRO DE MÊS ---
    if mes_selecionado:
